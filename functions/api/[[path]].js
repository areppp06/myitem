  ...
  try {
    ...
    if (path === "/places" && method === "POST") {
      ...
      const existing = await env.DB.prepare(
        "SELECT id, name FROM places WHERE LOWER(name) = LOWER(?)"
      )
        .bind(clean)
        .first();
      if (existing) {
        return error(`Place "${existing.name}" already exists`, 409, corsHeaders);
      }
    ...
    if (path === "/tags" && method === "POST") {
      ...
      const existing = await env.DB.prepare(
        ...
        .first();
      if (existing) {
        return error(`Tag "${existing.name}" already exists`, 409, corsHeaders);
      }
      const result = await env.DB.prepare(
    ...
    if (path === "/items" && method === "POST") {
      ...
      const result = await env.DB.prepare(
        ...
        .bind(name.trim(), place_id || null, categoryName, image_key || null, notes || null)
        .first();
    ...
    if (path.startsWith("/items/") && method === "PUT") {
      ...
      await env.DB.prepare(
        ...
        .bind(name?.trim(), place_id || null, categoryName, image_key || null, notes || null, id)
        .run();
